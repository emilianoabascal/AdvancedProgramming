...
void copyPGM(const pgm_t * source, pgm_t * destination)
{
    ...
    for (int i=0; i<source->image.height; i++)
    {
        for (int j=0; j<source->image.width; j++)
        {
            ...
            destination->image.pixels[i][j].value[R] = source->image.pixels[i][j].value[R];
            destination->image.pixels[i][j].value[G] = source->image.pixels[i][j].value[G];
            destination->image.pixels[i][j].value[B] = source->image.pixels[i][j].value[B];
        }
    }
}
//
...
void readPGMFile(const char * filename, pgm_t * pgm_image)
{
    ...
    if ( !strncmp(pgm_image->magic_number, "P3", 3) )
    {
...
void readPGMTextData(pgm_t * pgm_image, FILE * file_ptr)
{
    ...
    for (int i=0; i<pgm_image->image.height; i++)
    {
        for (int j=0; j<pgm_image->image.width; j++)
        {
            ...
            fscanf(file_ptr, "%hhu", &(pgm_image->image.pixels[i][j].value[R]));
            fscanf(file_ptr, "%hhu", &(pgm_image->image.pixels[i][j].value[G]));
            fscanf(file_ptr, "%hhu", &(pgm_image->image.pixels[i][j].value[B]));
        }
    }
}
    ...
    if ( fread(data, sizeof (unsigned char), pixels * 3, file_ptr) )
    {
        ...
        for (int i=0; i<pgm_image->image.height; i++)
        {
            for (int j=0; j<pgm_image->image.width; j++)
            {
                ...
                pgm_image->image.pixels[i][j].value[R] = data[k++];
                pgm_image->image.pixels[i][j].value[G] = data[k++];
                pgm_image->image.pixels[i][j].value[B] = data[k++];
            }
        }
    }
    else
...
void writePGMTextData(const pgm_t * pgm_image, FILE * file_ptr)
{
    ...
    for (int i=0; i<pgm_image->image.height; i++)
    {
        for (int j=0; j<pgm_image->image.width; j++)
        {
            fprintf(file_ptr, "%3hhu ", pgm_image->image.pixels[i][j].value[R]);
            fprintf(file_ptr, "%3hhu ", pgm_image->image.pixels[i][j].value[G]);
            fprintf(file_ptr, "%3hhu\t", pgm_image->image.pixels[i][j].value[B]);
...
void writePGMBinaryData(const pgm_t * pgm_image, FILE * file_ptr)
{
    ...
    for (int i=0; i<pgm_image->image.height; i++)
    {
        ...
        {
            // Copy the data into the correct structure
            data[k++] = pgm_image->image.pixels[i][j].value[R];
            data[k++] = pgm_image->image.pixels[i][j].value[G];
            data[k++] = pgm_image->image.pixels[i][j].value[B];
        }
    }
